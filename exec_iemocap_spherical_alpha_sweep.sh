...
do
  ...
  do
    echo "mode=sdt_cse_learnable_angles residual=spherical selection=validation alpha=${alpha} seed=${seed}"
    python -u "${SCRIPT_DIR}/train.py" \
      ...
      --spherical-attention-alpha-init "${alpha}" \
      --spherical-mlp-alpha-init "${alpha}" \
      --device cuda \
      --gpu-id "${GPU_ID}" \
      --seed "${seed}" \
      --epochs 150 \
